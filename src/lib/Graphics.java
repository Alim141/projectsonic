...
public final class Graphics implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static final Graphics std = new Graphics();
    // current pen color
    ...
    private static void validate(double v, String name) {
        if (Double.isNaN(v)) throw new IllegalArgumentException(name + " is NaN");
        if (Double.isInfinite(v)) throw new IllegalArgumentException(name + " is infinite");
    }

    // throw an IllegalArgumentException if s is null
    private static void validateNonnegative(double v, String name) {
        if (v < 0) throw new IllegalArgumentException(name + " negative");
    }
    ...
    /**
     * By Airat Ishimbaev at summer 2023
     * Returns the default scale, which is YMAX
     *
     * @return the default scale, which is YMAX
     */
     ...
     * (<em>x</em><sub>3</sub>, <em>y</em><sub>3</sub>).
     *
     * @param x1 the <em>x</em>-coordinate of the first point
     * @param y1 the <em>y</em>-coordinate of the first point
     * @param x2 the <em>x</em>-coordinate of the second point
     * @param y2 the <em>y</em>-coordinate of the second point
     * @param x3 the <em>x</em>-coordinate of the third point
     * @param y3 the <em>y</em>-coordinate of the third point
     * @throws IllegalArgumentException if any coordinate is either NaN or infinite
     */
    public static void triangle(double x1, double y1, double x2, double y2, double x3, double y3) {
        ...
        GeneralPath path = new GeneralPath();
        path.moveTo((float) scaleX(x1), (float) scaleY(y1));
        path.lineTo((float) scaleX(x2), (float) scaleY(y2));
        path.lineTo((float) scaleX(x3), (float) scaleY(y3));
        path.closePath();
        offscreen.draw(path);
        draw();
    }
    ...
    /**
     * Draws a filled triangle with the vertices
     * (<em>x</em><sub>1</sub>, <em>y</em><sub>1</sub>),
     * (<em>x</em><sub>2</sub>, <em>y</em><sub>2</sub>),
     * (<em>x</em><sub>3</sub>, <em>y</em><sub>3</sub>).
     *
    ...
    private static Image getImage(String filename) {
        ...
        if (icon.getImageLoadStatus() != MediaTracker.COMPLETE) {
            URL url = Graphics.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
    ...
    public static void main(String[] args) {
        Graphics.square(0.2, 0.8, 0.1);
        Graphics.filledSquare(0.8, 0.8, 0.2);
        Graphics.circle(0.8, 0.2, 0.2);

        Graphics.setPenColor(Graphics.RED);
        Graphics.setPenRadius(0.02);
        Graphics.arc(0.8, 0.2, 0.1, 200, 45);

        // draw a blue diamond
        Graphics.setPenRadius();
        Graphics.setPenColor(Graphics.BLUE);
        double[] x = {0.1, 0.2, 0.3, 0.2};
        double[] y = {0.2, 0.3, 0.2, 0.1};
        Graphics.filledPolygon(x, y);

        // text
        Graphics.setPenColor(Graphics.BLACK);
        Graphics.text(0.2, 0.5, "black text");
        Graphics.setPenColor(Graphics.WHITE);
        Graphics.text(0.8, 0.8, "white text");
        Graphics.filledTriangle(0.1, 0.2, 0.3, 0.2, 0.2, 0.3);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(Graphics.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            Graphics.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (MOUSE_LOCK) {
            mouseX = Graphics.userX(e.getX());
            mouseY = Graphics.userY(e.getY());
            isMousePressed = true;
        }
    }
    ...
    private static class RetinaImageIcon extends ImageIcon {
        ...
        public synchronized void paintIcon(Component c, java.awt.Graphics g, int x, int y) {
